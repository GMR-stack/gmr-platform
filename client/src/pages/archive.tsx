        ...
        <div className="max-w-6xl mx-auto flex items-center justify-between gap-4 flex-wrap px-6 py-3">
          ...
          <div className="flex items-center gap-2">
            ...
            {isGuest ? (
              <>
                <Link href="/login">
                  <Button variant="outline" size="sm" className="text-xs h-7">Log In</Button>
                </Link>
                <Button
                  size="sm"
        ...
        {isLoading ? (
          ...
          <div className="space-y-3">
            {paginatedReports.map((report) => {
              ...
              return (
                <Card
                  key={report.id}
                  className={`p-5 hover-elevate cursor-pointer transition-colors ${!canRead ? "opacity-80 hover:opacity-100" : ""}`}
                  onClick={handleClick}
          ...
          <Card className="p-8 text-center">
            ...
            <p className="text-sm text-muted-foreground" data-testid="text-no-reports">
              {filterType !== "all" ? `No ${reportTypeLabel(filterType)} reports found.` : "No reports available yet."}
            </p>
          </Card>
        )}
      </main>
          ...
          {isGuest ? (
            ...
            <div className="space-y-3 pt-2">
              ...
              <Link href="/login?mode=signup">
                <Button className="w-full bg-[#1a1f36] hover:bg-[#2a2f46] text-white">
                  Create Account & Subscribe
                </Button>
              </Link>
              <div className="text-center">
                <Link href="/login">
                  <span className="text-xs text-muted-foreground hover:text-foreground underline cursor-pointer">
